...
import {hasTwoChars, zeroToHundred, ifTrueFalse, isTrue, isDefined} from '../Utility.js';
...
const PlayerRowEdit = ({player, toggleEditing, editPlayer}) => {
    ...
    let fn_classes = getClassesIf(hasTwoChars(first_name));
    let ln_classes = getClassesIf(hasTwoChars(last_name));
    let score_classes = getClassesIf(scoreIsValid() && isDefined(score));

    // When the user attempts to save the changes...
    const attemptToSave = () => {
        let checks = [hasTwoChars(first_name), hasTwoChars(last_name), scoreIsValid(), isDefined(score)];
            ...
            <PlayerListCell>
                <div className="form__groups--flex">
                    <InputGroup>
                        ...
                        <input id="row__first_name" className={fn_classes.join(' ')} type="text" value={first_name} onChange={setFNEvent} placeholder="First Name" required />
                    </InputGroup>
                    <InputGroup>
                        <label htmlFor="row__last_name">Last Name</label>
                        <input id="row__last_name" className={ln_classes.join(' ')} type="text" value={last_name} onChange={setLNEvent} placeholder="Last Name" required />
                    </InputGroup>
                </div>
            </PlayerListCell>
            <PlayerListCell>
